...
const KillerSudokuPage = () => {
	...
	const [difficulty, setDifficulty] = useState<SudokuDifficulty>(
		(String(router.query.d) as any) || 'Easy'
	)
	...
	useEffect(() => {
		...
		generateKillerSudoku(diff, true)
	}, [router])
		...
		const generate = () => {
			...
			pb.setProgress({
				...
				onAnimationEnd() {
					...
					// 	difficulty: 'Extreme',
					// 	solution: [
					// 		4, 6, 5, 9, 8, 2, 1, 3, 7, 1, 7, 9, 4, 6, 3, 2, 5, 8, 3, 8, 2, 7, 5,
					// 		1, 4, 9, 6, 2, 9, 8, 3, 1, 7, 6, 4, 5, 5, 4, 6, 8, 2, 9, 7, 1, 3, 7,
					// 		3, 1, 6, 4, 5, 8, 2, 9, 9, 1, 7, 2, 3, 8, 5, 6, 4, 6, 2, 3, 5, 7, 4,
					// 		9, 8, 1, 8, 5, 4, 1, 9, 6, 3, 7, 2,
					// 	],
					// }
	...
	const eraseNumber = () => {
		...
		const ans = [
			...answer.filter(
				(v) => !(v.col == selectedGrid.col && v.row == selectedGrid.row)
			),
		]
		checkAnswer(ans)
		setAnswer(ans)
	...
	return (
		<>
			...
			<div className={'killer-sudoku-page ' + config.deviceType}>
				...
				<div className={'ks-main  ' + config.deviceType}>
					<div
						style={
							{
								'--width':
									...
									(config.window.width <= 640
										? config.window.width <= 460
											...
											: config.window.width * 0.7
										: (config.window.width > 880 ? 880 : config.window.width) *
										  (config.deviceType === 'Mobile' ? 0.552 : 0.592)) + 'px',
							} as any
						}
					...
					>
						{[1, 2, 3, 4, 5, 6, 7, 8, 9, 'Pen', 'Erase', 'Undo'].map((v, i) => {
							return (
								<div key={i} className='k-i-item'>
									<saki-button
										...
										border={
											(typeof v === 'string' && v === 'Pen' && isOpenNote) ||
											inputNum === v
												? '3px solid var(--saki-default-color)'
												: 'none'
										}
										width={
										...
										ref={bindEvent({
											tap: () => {
												if (typeof v === 'number') {
													...
													fillNumber(selectedGrid.row, selectedGrid.col, v)
													return
												}
												switch (v) {
													case 'Erase':
